  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-base px-4 py-8">
      ...
      <div className="mb-8 sm:mb-12 text-center">
        <h1
          className="text-4xl sm:text-6xl font-display font-bold text-gold mb-2"
            ...
            <>
              <div className="mt-2 space-y-1.5 max-h-[40vh] overflow-y-auto">
                {localHistory.slice(0, 20).map((record, i) => {
                  ...
                  const resultLabel = record.result === 'draw' ? '무' : record.result === 'win' ? '승' : '패';
                    ...
                    <div key={i} className="bg-panel/60 rounded-lg border border-white/5 px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {record.comboNames.length > 0 && (
                            <p className="text-[10px] text-text-secondary mt-0.5 truncate">
                              {record.comboNames.join(', ')}
                            </p>
                          )}
                        </div>
                        {record.score > 0 && (
      ...
      <div className="bg-panel rounded-2xl border border-white/5 shadow-panel p-5 sm:p-8 w-full max-w-md mb-4 sm:mb-6">
        ...
        <div className="mb-5 sm:mb-6">
          ...
          <p className="text-[10px] sm:text-xs text-text-muted mt-2">
            {difficulty === 'easy' && 'AI가 랜덤하게 플레이합니다. 고스톱 입문용!'}
            {difficulty === 'normal' && 'AI가 기본 전략으로 플레이합니다.'}
            {difficulty === 'hard' && 'AI가 카드 카운팅과 전략적 판단을 합니다.'}
          </p>
        </div>
        ...
        <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
          <ToggleOption
            label="효과음"
            description="카드 놓기, 먹기, 고/스톱 효과음"
            checked={soundEnabled}
            onChange={setSoundEnabled}
          />
          <ToggleOption
            label="가이드 표시"
            description="점수, 족보 진행 상황을 표시"
            checked={showHints}
      ...
      <button
        ...
        className="text-xs text-text-muted hover:text-gold transition-colors cursor-pointer mt-2"
      >
        규칙
      </button>
      ...
      <div className="mt-4 sm:mt-6 text-center text-[10px] sm:text-xs text-text-muted space-y-1">
        <p>한국식 고스톱 (3인 플레이) · AI 대전</p>
        <p>48장 화투패 · 광/띠/열끗/피</p>
      </div>
    </main>
  );
}
